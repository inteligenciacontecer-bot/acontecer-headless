...
async function getDiputados(): Promise<any[]> {
  try {
...
export default async function ComisionesPage() {
  const diputados: any[] = await getDiputados();

  // Construir índice invertido: nombre_comision → [diputado]
  const comisionMap: Record<string, any[]> = {};
  let hayDatos = false;

  for (const d of diputados) {
    if (!d.comisiones) continue;
    const lista = d.comisiones
      .split(/[,;|\n]/)
      .map((c: string) => c.trim())
      .filter(Boolean);
    if (lista.length) hayDatos = true;
    for (const nombre of lista) {
      if (!comisionMap[nombre]) comisionMap[nombre] = [];
      comisionMap[nombre].push(d);
    }
  }

  // Ordenar por nombre de comisión (permanentes primero)
  const comisiones = Object.entries(comisionMap)
    .sort(([a], [b]) => {
      const aPerm = a.toLowerCase().includes('permanente');
      const bPerm = b.toLowerCase().includes('permanente');
    ...
    <>
      ...
      <div style={{ background: 'linear-gradient(150deg, #00007a 0%, #0000C8 60%, #0a73ce 100%)', padding: '28px 16px 22px' }}>
        <div style={{ maxWidth: 1140, margin: '0 auto' }}>
          ...
          <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: 13, margin: 0, maxWidth: 540 }}>
            Composición de las comisiones permanentes y especiales de la Asamblea Legislativa de Costa Rica — período 2026–2030.
          </p>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1140, margin: '0 auto', padding: '20px 16px 56px' }}>
        ...
        {!hayDatos ? (
          /* Estado vacío — datos aún no sincronizados */
          <div style={{ background: 'white', borderRadius: 16, padding: '48px 24px', textAlign: 'center', boxShadow: '0 2px 16px rgba(0,0,0,0.06)', border: '1px solid #f0f0f0' }}>
            <svg viewBox="0 0 24 24" fill="none" stroke="#0000A2" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round" style={{width:40,height:40,marginBottom:16}}>
            ...
            <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
              <Link href="/asamblea" style={{
                background: '#0000A2', color: 'white', padding: '10px 20px', borderRadius: 10,
                textDecoration: 'none', fontSize: 13, fontWeight: 700,
              }}>
                ← Volver al Dashboard
              </Link>
              <Link href="/asamblea/diputados" style={{
                background: '#f1f5f9', color: '#334155', padding: '10px 20px', borderRadius: 10,
                textDecoration: 'none', fontSize: 13, fontWeight: 700,
              }}>
                Ver Diputados
              </Link>
          ...
          <>
            ...
            <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
              <div style={{ background: 'white', borderRadius: 12, padding: '12px 20px', boxShadow: '0 1px 8px rgba(0,0,0,0.06)', border: '1px solid #f0f0f0', display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  <div style={{ fontFamily: 'var(--font-Lora), serif', fontSize: 24, fontWeight: 900, color: '#0000A2', lineHeight: 1 }}>
                    {comisiones.length}
                  </div>
              ...
              <div style={{ background: 'white', borderRadius: 12, padding: '12px 20px', boxShadow: '0 1px 8px rgba(0,0,0,0.06)', border: '1px solid #f0f0f0', display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 11, color: '#94a3b8', fontWeight: 600, marginTop: 2 }}>Diputados en registro</div>
                </div>
              </div>
            </div>
                  ...
                  <div style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      <div>
                        ...
                        {nombre.toLowerCase().includes('permanente') && (
                          ...
                          <div style={{
                            ...
                            <div style={{ minWidth: 0 }}>
                              ...
                              <div style={{ fontSize: 10, color, fontWeight: 600, marginTop: 2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                {(d.fraccion || d.partido || '').split(' ').filter((w: string, i: number, a: string[]) => i < 2 || w.length > 3).join(' ')}
                              </div>
                            </div>
                          </div>
                        </Link>
